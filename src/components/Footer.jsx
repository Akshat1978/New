...
const Footer = () => {
  return (
    <footer className={`${styles.paddingY} ${styles.flexCenter} flex-col relative`}>
      <div className="absolute z-0 w-screen h-full bg-[#06080d]" />
      <div className={`${styles.flexStart} md:flex-row flex-col mb-8 w-full z-10`}>
        <div className="flex flex-col justify-start flex-1 mr-10">
        ...
        <div className="flex flex-row flex-wrap justify-between flex-[1.5] w-full mt-10 md:mt-0">
          {footerLinks.map((footerLink) => (
            <div key={footerLink.key} className="flex flex-col ss:my-0 my-4 min-w-[150px] gap-4">
              <h4 className="text-lg font-medium leading-7 text-white font-poppins">
                {footerLink.title}
              </h4>
              <ul className="list-none">
                {footerLink.links.map((link) => (
                  <li
                    key={link.name} // Ensure each list item has a unique key
      ...
      <div className="border-t-[#3F3E45] z-10 pt-6 border-t-[1px] flex flex-col items-center justify-between w-full gap-10 md:flex-row">
        ...
        <div className="flex flex-row mt-6 md:mt-0">
          {socialMedia.map((social) => (
            <img
              key={social.id} // Ensure each social media icon has a unique key
              src={social.icon}
              alt={social.id}
              className="w-5 h-5 object-contain cursor-pointer mr-6 last:mr-0" // Adjusted styling for spacing
            />
          ))}
        </div>
      </div>
